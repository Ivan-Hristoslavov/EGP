...
export function generateInvoicePDF(
  ...
  if (
    invoice.customer?.customer_type === "company" &&
  ...
  const serviceText =
    ...
    "Professional Aesthetic Treatment";
  ...
  serviceLines.forEach((line, index) => {
    const wrappedServiceText = doc.splitTextToSize(
      line.trim(),
      tableWidth - 200,
    );
  ...
  if (vatSettings?.is_enabled && invoice.vat_amount > 0) {
    ...
    doc.text(`£${invoice.subtotal.toFixed(2)}`, pageWidth - 40, y, {
      align: "right",
    });

    y += spacing.lineHeight;
    doc.text(`VAT @ ${invoice.vat_rate}%`, pageWidth - 180, y);
    doc.text(`£${invoice.vat_amount.toFixed(2)}`, pageWidth - 40, y, {
      align: "right",
    });
  ...
  doc.text(`£${invoice.total_amount.toFixed(2)}`, pageWidth - 40, y, {
    align: "right",
  });
    ...
    notes.forEach((note) => {
      if (note.trim()) {
        ...
        const wrappedNotes = doc.splitTextToSize(
          note.trim(),
          notesContainerWidth - 20,
        );

        wrappedNotes.forEach((line: string) => {
          if (notesY < y + notesContainerHeight - 10) {
            // Check if we're still within container

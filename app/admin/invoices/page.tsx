...
export default function InvoicesPage() {
  ...
  const loadData = async (page: number = 1) => {
    try {
      ...
      if (invoicesRes.ok) {
        ...
        const processedInvoices = (invoicesData.invoices || []).map(
          (invoice: any) => {
            if (
              invoice.image_attachments &&
              typeof invoice.image_attachments === "string"
            ) {
              try {
                invoice.image_attachments = JSON.parse(
                  invoice.image_attachments,
                );
              } catch (e) {
      ...
      showError(
        ToastMessages.general.error.title,
        ToastMessages.general.error.message,
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateInvoice = async (invoiceData: FormData) => {
    try {
      ...
      if (response.ok) {
        ...
        showSuccess(
          ToastMessages.invoices.created.title,
          ToastMessages.invoices.created.message,
        );
      } else {
      ...
      showError(
        ToastMessages.invoices.error.title,
        error instanceof Error
          ? error.message
          : ToastMessages.invoices.error.message,
      );
    } finally {
  ...
  const handleSendInvoice = async (
    ...
    try {
      ...
      const response = await fetch(
        `/api/invoices/${selectedInvoice.id}/send-email`,
        {
          method: "POST",
          headers: {
  ...
  const handleDownloadInvoice = async (invoice: Invoice) => {
    ...
    try {
      ...
      showSuccess(
        ToastMessages.invoices.downloaded.title,
        ToastMessages.invoices.downloaded.message,
      );
    } catch (error) {

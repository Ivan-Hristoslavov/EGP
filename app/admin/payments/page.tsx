...
export default function PaymentsPage() {
  ...
  const [customersForPayment, setCustomersForPayment] = useState<
    {
      id: string;
      first_name: string;
      last_name: string;
      ...
      try {
        ...
        if (value && typeof value === "object" && !cancelled) {
          setDepositSettings({
            ...
            percentage:
              value.percentage != null ? Number(value.percentage) : 50,
            fixedAmount:
  ...
  useEffect(() => {
    if (editingPayment) {
      setPaymentFormData({
        ...
        payment_type:
          (editingPayment.payment_type as "full" | "deposit") || "full",
      ...
      if (response.ok) {
        ...
        setTotalPages(
          data.pagination?.totalPages ||
            Math.ceil(fetchedPayments.length / limit),
        );
      } else {
          ...
          <CardBody className="p-4 sm:p-5">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <p className="text-xs sm:text-sm font-medium text-default-500">
                  Total Received
                </p>
          ...
          <CardBody className="p-4 sm:p-5">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-lg sm:text-xl font-bold text-[#464C45] dark:text-egp-beige">
                        ...
                        <td className="px-3 py-2.5 min-w-0">
                          <div className="min-w-0 overflow-hidden">
                            ...
                            <div
                              ...
                              title={payment.customers?.email || "No email"}
                            >
            ...
            <>
              ...
              <ModalBody>
                <p className="text-default-600">
                  ...
                  <strong>£{paymentToDelete?.amount.toFixed(2)}</strong> for{" "}
                  <strong>
                    {paymentToDelete?.customers?.name || "Unknown Customer"}
                  </strong>
              ...
              <ModalBody className={formLayout.modalBody}>
                <div className={formLayout.sectionGap}>
                  <div className={formLayout.gridFields}>
                    ...
                    >
                      {customersForPayment.map((c) => (
                        <SelectItem
                          key={c.id}
                          textValue={`${c.first_name} ${c.last_name}`}
                        >
                          <div>
                    ...
                    {paymentFormData.customer_id &&
                      (() => {
                        const c = customersForPayment.find(
                          (x) => x.id === paymentFormData.customer_id,
                        );
                      ...
                      {(paymentFormData.customer_id
                        ? bookingsForPayment.filter(
                            (b) =>
                              ...
                              b.customer_email ===
                                customersForPayment.find(
                                  (c) => c.id === paymentFormData.customer_id,
                                )?.email,
                          )
                        : bookingsForPayment
                      ).map((b) => (
                  ...
                  <Input
                    ...
                    onValueChange={(v) =>
                      setPaymentFormData((prev) => ({ ...prev, notes: v }))
                    }
                  />
                </div>
              </ModalBody>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-base font-semibold">
                        {selectedPayment.customers?.name || "Unknown Customer"}
                      </p>
                      <p className="text-sm text-default-500">
                        {selectedPayment.customers?.email || "No email"}
                      </p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-default-500">
                      ...
                      <p className="text-base font-semibold">
                        {selectedPayment.bookings?.service || "Manual Payment"}
                      </p>
                    ...
                    <div>
                      ...
                      {selectedPayment.payment_type === "deposit" &&
                        selectedPayment.bookings?.remaining_amount != null &&
                        selectedPayment.bookings.remaining_amount > 0 && (
                          <p className="text-sm text-warning mt-1">
                            £
                            {Number(
                              selectedPayment.bookings.remaining_amount,
                            ).toFixed(2)}{" "}
                            due on arrival
                          </p>
                        )}
                    ...
                    {(selectedPayment.bookings?.booking_number ||
                    ...
                    {selectedPayment.reference &&
                      selectedPayment.reference.startsWith("pi_") && (
                        <div>
                          <label className="text-sm font-medium text-default-500">
                            Payment Reference
                          </label>
                    ...
                    {(selectedPayment.reference?.startsWith("plink_") ||
                      (selectedPayment.notes &&
                        /https?:\/\//.test(selectedPayment.notes))) && (
                      <div>
                        <label className="text-sm font-medium text-default-500 mb-2 block">
                          Payment Link
                        </label>
                        {(() => {
                          const urlMatch = selectedPayment.notes?.match(
                            /URL:\s*(https?:\/\/[^\s|]+)/,
                          );
                          const paymentUrl = urlMatch
                            ? urlMatch[1].trim()
                            : null;

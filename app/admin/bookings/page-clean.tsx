  ...
  const loadBookings = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const bookingsToUse =
          fetchedBookings.length > 0 ? fetchedBookings : DUMMY_BOOKINGS;

        setBookings(bookingsToUse);
        setTotalCount(data.totalCount || bookingsToUse.length);
        setTotalPages(
          Math.ceil((data.totalCount || bookingsToUse.length) / limit),
        );
      } else {
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredBookings.map((booking) => (
                  <tr key={booking.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${getPaymentStatusColor(booking.payment_status)}`}
                      >

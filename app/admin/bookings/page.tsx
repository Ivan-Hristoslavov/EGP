            ...
            (s: {
              ...
              discounted_price:
                s.discounted_price != null
                  ? typeof s.discounted_price === "number"
                    ? s.discounted_price
                    : parseFloat(String(s.discounted_price))
                  : null,
            }),
          );
    ...
    if (dateFilter && dateFilter !== "all") {
      ...
      switch (dateFilter) {
        ...
        case "next_month":
          ...
          matchesDate =
            bookingDate.getMonth() === nextMonth.getMonth() &&
  ...
  const validateForm = () => {
    ...
    if (formData.payment_method === "cash_and_card") {
      ...
      if (
        cashAmt > 0 &&
        cardAmt > 0 &&
        Math.abs(cashAmt + cardAmt - total) > 0.01
      ) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const url = editingBooking
        ? `/api/bookings?id=${editingBooking.id}`
        : "/api/bookings";
    ...
    fetch(`/api/payments?booking_id=${selectedBooking.id}`)
      ...
      .then((data) => {
        const list = (data.payments || []).map(
          (p: { payment_method: string; amount: number }) => ({
            ...
            <>
              ...
              >
                {filteredBookings.map((booking) => (
                  <Card
                    key={booking.id}
                    className="border border-divider shadow-sm overflow-visible"
                  >
                    <CardBody className="p-4 sm:p-5 space-y-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          {booking.customer_email && (
                            <a
                              className="text-sm text-primary flex items-center gap-1 mt-0.5 truncate"
                          ...
                          {booking.customer_phone && (
                            <a
                              className="text-sm text-success flex items-center gap-1 mt-0.5 truncate"
                ...
                <table className="w-full min-w-[640px] table-fixed">
                  <thead className="sticky top-0 z-10 bg-default-100 border-b border-divider">
                    <tr>
                      ...
                      <th className="w-[18%] px-2 py-3 text-center text-xs font-semibold text-default-600 uppercase tracking-wider">
                        Service
                      </th>
                      <th className="w-[14%] px-2 py-3 text-center text-xs font-semibold text-default-600 uppercase tracking-wider">
                        Date & Time
                      </th>
                      <th className="w-[12%] px-2 py-3 text-center text-xs font-semibold text-default-600 uppercase tracking-wider">
                        Status
                      </th>
                      <th className="w-[12%] px-2 py-3 text-center text-xs font-semibold text-default-600 uppercase tracking-wider">
                        Payment
                      ...
                      <th className="w-[10%] px-2 py-3 text-center text-xs font-semibold text-default-600 uppercase tracking-wider">
                        Actions
                      </th>
                    </tr>
                  </thead>
                            ...
                            <div className="text-xs text-default-500 truncate w-full text-center">
                              {booking.customer_email ? (
                                <button
                                  ...
                                  onClick={() =>
                                    handleEmailCustomer(booking.customer_email)
                                  }
                                >
                            ...
                            <div className="text-xs text-default-500 truncate w-full text-center">
                              {booking.customer_phone ? (
                                <button
                                  ...
                                  onClick={() =>
                                    handleCallCustomer(booking.customer_phone)
                                  }
                                >
                            ...
                            (booking.amount_paid != null ||
                              ...
                              <>
                                ...
                                <span className="text-xs font-normal">
                                  £{(booking.remaining_amount ?? 0).toFixed(2)}{" "}
                                  due
                                </span>
                              </>
                            ) : (
            ...
            <>
              ...
              <ModalBody>
                <p className="text-default-600">
                  ...
                  <strong>{bookingToDelete?.customer_name}</strong>? This action
                  cannot be undone.
                </p>
              </ModalBody>
              <ModalFooter>
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody
                className={`${formLayout.modalBody} overflow-y-auto max-h-[min(70vh,500px)] sm:max-h-[min(75vh,600px)]`}
              >
                <form className={formLayout.sectionGap} onSubmit={handleSubmit}>
                    ...
                    <Select
                      ...
                      onSelectionChange={(keys) => {
                        ...
                        const displayAmount = svc
                          ? (svc.discounted_price != null &&
                            svc.discounted_price > 0
                  ...
                  <div className="flex items-center gap-3 p-3 bg-default-50 rounded-lg border border-divider">
                    ...
                    <label
                      ...
                      htmlFor="booking-deposit-toggle"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Select
                      ...
                      onSelectionChange={(keys) => {
                        ...
                        handleInputChange({
                          target: { name: "payment_status", value: selected },
                        } as any);
                      }}
                    >
                      <>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      (selectedBooking.amount_paid != null ||
                        selectedBooking.remaining_amount != null) ? (
                        <div className="space-y-1">
                          ...
                          <p className="text-base text-default-600">
                            £
                            {(selectedBooking.remaining_amount ?? 0).toFixed(2)}{" "}
                            due on arrival
                          </p>
                        </div>
                      ) : (
                        <>
                          ...
                          >
                            £
                            {typeof selectedBooking.amount === "number"

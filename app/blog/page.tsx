  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900">
      <div className={`${layout.container} pt-20 sm:pt-24 pb-8 sm:pb-12`}>
        ...
        <p
          className={`${typography.lead} font-montserrat font-light max-w-2xl mx-auto text-center text-xs sm:text-sm mb-4 sm:mb-8`}
        >
          Stay informed about the latest in aesthetic treatments, skincare tips,
          and beauty insights.
        </p>
        ...
        {posts.length === 0 ? (
          <Card className="py-12">
            <CardBody className="text-center">
              ...
              <p className="text-default-500">Check back soon for updates!</p>
            </CardBody>
          </Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
            {featuredPosts.map((post) => (
              <Link key={post.id} href={`/blog/${post.slug}`}>
                <Card
                  isPressable
                  className="group relative flex h-full flex-col md:col-span-2"
                  shadow="md"
                >

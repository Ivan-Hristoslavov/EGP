...
export default async function AboutPage() {
  ...
  const heroSection = sections.find(
    (s) =>
      s.image_url && (s.section_type === "hero" || s.section_type === "story"),
  );
      ...
      {otherSections.length > 0 && (
        <div className="w-full max-w-4xl mx-auto px-6 sm:px-10 lg:px-16 py-16 lg:py-24 space-y-20">
          {otherSections.map((section) => {
            const label =
              sectionLabel[section.section_type] || section.section_type;
                ...
                {section.bullet_points && section.bullet_points.length > 0 && (
                  <ul className="mt-6 space-y-3">
                    {section.bullet_points.map((bullet, idx) => (
                      ...
                      >
                        <span className="text-[#9d9585] dark:text-[#b5ad9d] mt-0.5 font-bold">
                          ✓
                        </span>
                        <span>{bullet}</span>
                      </li>

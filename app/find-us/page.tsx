...
export default function FindUsPage() {
  ...
  if (
    displayFindUsData.transportOptions.tube &&
    displayFindUsData.transportOptions.tube.length > 0
  ) {
    transportOptions.push({
      type: "Tube/Underground",
      icon: <Train className="w-5 h-5" />,
      details: displayFindUsData.transportOptions.tube.map(
        (t) =>
  ...
  if (
    displayFindUsData.transportOptions.bus &&
    displayFindUsData.transportOptions.bus.length > 0
  ) {
    transportOptions.push({
      type: "Bus",
      icon: <Bus className="w-5 h-5" />,
      details: displayFindUsData.transportOptions.bus.map(
        (b) =>
  ...
  if (
    displayFindUsData.transportOptions.car &&
    displayFindUsData.transportOptions.car.length > 0
  ) {
    transportOptions.push({
      type: "Car/Parking",
      icon: <Car className="w-5 h-5" />,
      details: displayFindUsData.transportOptions.car.map(
        (c) =>
  ...
  if (
    displayFindUsData.transportOptions.walking &&
    displayFindUsData.transportOptions.walking.length > 0
  ) {
    transportOptions.push({
      type: "Walking",
      icon: <Navigation className="w-5 h-5" />,
      details: displayFindUsData.transportOptions.walking.map(
        (w) => `${w.distance} from ${w.from}`,
      ),
    });
  }
  ...
  const handleDirections = () => {
    const addressToUse =
      displayContactInfo.googleMapsAddress || displayContactInfo.address;
    ...
    <div className="min-h-screen bg-[#f5f1e9] dark:bg-gray-900 pt-16 sm:pt-20 pb-6 sm:pb-10">
      <div className={layout.container}>
        ...
        <div className="relative flex items-center justify-between mb-2 sm:mb-4">
          ...
          <h1 className="absolute left-1/2 -translate-x-1/2 text-lg sm:text-xl md:text-2xl font-bold text-[#3a3428] dark:text-white font-montserrat">
            Find Us
          </h1>
          <div aria-hidden className="w-14 sm:w-16" />
        </div>
        <p
          className={`${typography.lead} text-[#6f6652] dark:text-gray-300 font-montserrat font-light max-w-3xl mx-auto text-xs sm:text-sm text-center mb-3 sm:mb-4`}
        >
        ...
        <div className="flex justify-center items-center mb-3 sm:mb-5 w-full px-1">
          <Tabs
            ...
            classNames={{
              ...
              tabList:
                "gap-1 w-full relative rounded-lg p-1 bg-default-100/50 justify-center",
              cursor: "w-full bg-[#3a3428] dark:bg-[#d8c5a7]",
              tab: "flex-1 justify-center px-2 sm:px-3 h-9 sm:h-10 min-w-0",
              tabContent:
                "group-data-[selected=true]:text-white flex items-center justify-center gap-1 text-xs sm:text-sm",
            }}
            color="primary"
            selectedKey={activeTab}
            onSelectionChange={(key) =>
              setActiveTab(key as "location" | "contact" | "hours")
            }
          >
            <Tab
              key="location"
                      ...
                      <CardBody className="p-2 sm:p-4">
                        ...
                        <ul className="space-y-0.5 sm:space-y-1">
                          {option.details.map((detail, detailIndex) => (
                            <li
                              key={detailIndex}
                              className="flex items-start gap-1.5 text-default-500"
                            >
                              <div className="w-1 h-1 bg-[#9d8c6b] rounded-full mt-1.5 flex-shrink-0" />
                ...
                {nearbyLandmarks.length > 0 ? (
                  <>
                    <p className="text-default-500 mb-2 sm:mb-4 text-[11px] sm:text-xs leading-snug">
                      Our clinic at {displayContactInfo.address} is surrounded
                      by some of London&apos;s most iconic locations:
                    </p>
        ...
        {activeTab === "contact" && (
          <div className="space-y-3 sm:space-y-5 max-w-4xl mx-auto">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
              {/* Phone */}
              <Card
                className="border border-[#e4d9c8] dark:border-gray-700 hover:shadow-md transition-shadow"
                shadow="sm"
              >
                <CardBody className="text-center p-3 sm:p-4">
                  ...
                  <p className="text-[11px] sm:text-xs text-default-600 mb-2 break-all">
                    {displayContactInfo.phone}
                  </p>
                  <Button
                    className="bg-[#3a3428] text-[#f5f1e9] hover:bg-[#4a4438]"
                    size="sm"
                    onPress={handleCall}
                  >
                    Call Now
              ...
              >
                <CardBody className="text-center p-3 sm:p-4">
                  ...
                  <p className="text-[11px] sm:text-xs text-default-600 mb-2 break-all line-clamp-2">
              ...
              >
                <CardBody className="text-center p-3 sm:p-4">
                  <div className="bg-[#e8e1d4] dark:bg-[#3a3428] p-2 rounded-full w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-1.5 flex items-center justify-center flex-shrink-0">
                    <svg
                      className="w-5 h-5 sm:w-6 sm:h-6 text-[#6f6652] dark:text-[#d8c5a7]"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  <p className="text-[11px] sm:text-xs text-default-600 mb-2">
            ...
            <Card className="bg-[#3a3428] text-[#f5f1e9] shadow-sm">
              <CardBody className="text-center p-3 sm:p-5">
                ...
                <p className="text-[#d8c5a7] text-xs sm:text-sm mb-3">
                  For urgent bookings or consultations, please contact us
                  directly
                </p>
        ...
        {activeTab === "hours" && (
          <div className="space-y-3 sm:space-y-5 max-w-2xl mx-auto">
            <Card
              className="border border-[#e4d9c8] dark:border-gray-700"
              shadow="sm"
            >
              <CardHeader className="flex flex-col items-center justify-center text-center pb-0 px-3 sm:px-4 py-2 sm:py-3">
                <div className="flex flex-col items-center gap-1.5 sm:gap-2">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[11px] sm:text-xs text-default-500 mt-0.5">
                ...
                {loading ? (
                  ...
                  <div className="space-y-1">
                    {Object.entries(openingHours).map(([key, dayInfo]) => (
                      ...
                      >
                        <span className="font-medium text-foreground w-20 sm:w-24 text-center text-xs sm:text-sm">
                          {dayInfo.day}
                        </span>
                        <span className="text-xs text-default-600 text-center min-w-[7rem] sm:min-w-[8rem]">
                          {dayInfo.hours}
                        </span>
                        <Chip

...
export async function POST(
  ...
  try {
    ...
    for (const attachment of imageAttachments) {
      try {
        console.log(
          `Attempting to download attachment: ${attachment.filename} from URL: ${attachment.path}`,
        );
        ...
        if (attachment.path.startsWith("http")) {
          ...
          if (!response.ok) {
            console.error(
              `Failed to download image from URL: ${attachment.path}, status: ${response.status}`,
            );
            continue;
          }
          ...
          console.log(
            `Successfully downloaded file: ${attachment.filename}, size: ${fileContent.byteLength} bytes`,
          );
          ...
          console.log(
            `Successfully read local file: ${attachment.filename}, size: ${fileContent.length} bytes`,
          );
    ...
    console.log(
      `📎 Total attachments prepared: ${emailAttachments.length} (including PDF)`,
    );
    console.log(
      `📎 Attachment details:`,
    ...
    try {
      console.log(
        `Sending email to: ${invoice.customer.email} with ${emailAttachments.length} attachments`,
      );
        ...
        try {
          await sendEmail({
            ...
            html: emailHtml.replace(
              ...
              "Images were not included due to technical issues",
            ),
            attachments: [],
          });
          ...
          return NextResponse.json({
            message:
              "Invoice email sent successfully (without attachments due to technical issues)",
          ...
          console.error(
            "Error sending email even without attachments:",
            secondError,
          );

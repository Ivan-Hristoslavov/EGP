...
export async function POST(request: NextRequest) {
  try {
    ...
    const invoiceData = {
      ...
      company_vat_number:
        (formData.get("company_vat_number") as string) || null,
    ...
    if (images.length > 0) {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        try {
          ...
          const originalName = processedImage.file.name.replace(
            /[^a-zA-Z0-9.-]/g,
            "_",
          );
          ...
          const { data: bucketTest, error: bucketError } =
          ...
          imageAttachments.push({
            ...
            compressionRatio: processedImage.wasConverted
              ? ((file.size - processedImage.file.size) / file.size) * 100
              : 0,
          });
      ...
      console.log(
        `✅ Successfully processed ${imageAttachments.length}/${images.length} images`,
      );
    }
    ...
    if (numberError) {
      ...
      return NextResponse.json({ error: numberError.message }, { status: 400 });
    }

...
function extractFilePathFromUrl(url: string): string | null {
  ...
  try {
    ...
    if (
      bucketIndex !== -1 &&
      pathParts[bucketIndex + 1] === "v1" &&
      pathParts[bucketIndex + 2] === "object" &&
      pathParts[bucketIndex + 3] === "public"
    ) {
...
async function deleteImageFromStorage(imageUrl: string | null): Promise<void> {
  ...
  try {
    ...
    const { error } = await supabase.storage.from("egp").remove([filePath]);
...
export async function PUT(
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: "Failed to update press item" },
        { status: 500 },
      );
    }

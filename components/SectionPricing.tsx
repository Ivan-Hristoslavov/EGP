    ...
    <div className="group relative bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-200 dark:border-gray-600 p-8 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden flex flex-col h-full animate-pulse">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex items-center mb-6 flex-shrink-0">
          <div className="w-14 h-14 bg-gray-200 dark:bg-gray-700 rounded-2xl mr-4" />
        ...
        <div className="space-y-2 mb-6 flex-shrink-0">
          <div className="flex items-start">
            <div className="w-4 h-4 bg-gray-200 dark:bg-gray-700 rounded mr-2 mt-0.5" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 justify-center max-w-6xl mx-auto mb-16">
          {pricingCards.map((card, index) => (
            <div
              key={card.id}
              className="group relative bg-white dark:bg-egp-green rounded-3xl shadow-xl border-2 border-gray-300 dark:border-egp-green-dark p-8 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden flex flex-col h-full"
            >
              {/* Background Pattern */}
              <div
                className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${index % 2 === 0 ? "from-blue-100 to-purple-100" : "from-orange-100 to-red-100 dark:from-orange-900/30 dark:to-red-900/30"} rounded-full -translate-y-16 translate-x-16 group-hover:scale-150 transition-transform duration-500`}
              />
              <div
                className={`absolute bottom-0 left-0 w-24 h-24 bg-gradient-to-tr ${index % 2 === 0 ? "from-yellow-100 to-orange-100" : "from-green-100 to-blue-100 dark:from-green-900/30 dark:to-blue-900/30"} rounded-full translate-y-12 -translate-x-12 group-hover:scale-125 transition-transform duration-500`}
              />
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="flex items-center mb-6 flex-shrink-0">
                  <div
                    className={`w-14 h-14 bg-gradient-to-br ${index % 2 === 0 ? "from-blue-500 to-purple-600" : "from-orange-500 to-red-600 dark:from-orange-400 dark:to-red-500"} rounded-2xl flex items-center justify-center mr-4 group-hover:rotate-12 transition-transform duration-300`}
                  >
                    <svg
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {card.subtitle && (
                      <p
                        className={`${index % 2 === 0 ? "text-blue-600 dark:text-blue-400" : "text-orange-600 dark:text-orange-400"} font-medium text-sm truncate`}
                      >
                        {card.subtitle}
                      </p>
                    )}
                ...
                {card.table_rows && card.table_rows.length > 0 && (
                  <div
                    className={`bg-gradient-to-br ${index % 2 === 0 ? "from-gray-50 to-blue-50 dark:from-gray-700 dark:to-gray-600" : "from-gray-50 to-orange-50 dark:from-gray-800 dark:to-orange-900/20"} rounded-2xl p-4 mb-6 flex-1`}
                  >
                      ...
                      <table className="min-w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-200 dark:border-gray-500">
                            {card.table_headers &&
                              card.table_headers.map((header, headerIndex) => (
                                <th
                                  key={header}
                                  className={`py-2 pr-3 font-bold text-gray-900 dark:text-white text-xs sm:text-sm ${
                                    headerIndex === 0
                                      ? "text-left"
                                      : "text-center"
                                  }`}
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                          {card.table_rows.map((row, rowIndex) => (
                            ...
                            >
                              {card.table_headers &&
                                card.table_headers.map((header, cellIndex) => (
                                  <td
                                    key={header}
                                    className={`py-3 pr-3 font-medium transition-colors duration-300 text-xs sm:text-sm ${
                                      cellIndex === 0
                                        ? "text-gray-800 dark:text-gray-200 text-left"
                                        : cellIndex === 1
                                          ? (row[header]?.includes("£80")
                ...
                {card.notes && card.notes.length > 0 && (
                  <div className="space-y-2 mb-6 flex-shrink-0">
                    {card.notes.map((note, noteIndex) => (
                      <div
                        key={noteIndex}
                        className="flex items-start text-xs sm:text-sm text-gray-600 dark:text-gray-300 transition-colors duration-300"
                      >

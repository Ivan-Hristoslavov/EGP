...
function PricingCardModal({
  ...
  const defaultCard = {
    ...
    notes: [
      {
        icon: "✓",
        text: "Perfect for urgent repairs and smaller jobs",
        color: "green",
      },
    ] as PricingCardNote[],
    order: 0,
  ...
  const addTableHeader = () => {
    setFormData((prev: any) => ({
      ...
      table_headers: [
        ...prev.table_headers,
        `Column ${prev.table_headers.length + 1}`,
      ],
    }));
  };
  ...
  const removeTableHeader = (index: number) => {
    setFormData((prev: any) => {
      ...
      const newHeaders = prev.table_headers.filter(
  ...
  const updateTableRow = (
    index: number,
    field: keyof PricingCardTableRow,
    value: string,
  ) => {
    setFormData((prev: any) => ({
      ...prev,
          ...
          <div>
            ...
            <div className="space-y-3">
              {formData.table_rows.map((row: any, index: number) => (
                <div
                  key={index}
                  className="grid gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg"
                  style={{
                    gridTemplateColumns: `repeat(${formData.table_headers.length}, 1fr) auto`,
                  }}
                >
                  {formData.table_headers.map((header: string) => (
                    <input
                      ...
                      value={row[header] || ""}
                      onChange={(e) =>
                        updateTableRow(index, header, e.target.value)
                      }
                    />
                  ))}
                  <button
          ...
          <div>
            ...
            <div className="space-y-3">
              {formData.notes.map((note: any, index: number) => (
                <div
                  key={index}
                  className="grid grid-cols-1 md:grid-cols-4 gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg"
                >
                  <input
                    className="px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
...
export function AdminPricingManager({
    ...
    try {
      if (editingCard) {
        ...
        showSuccess(
          ...
          "Your pricing card has been updated successfully.",
        );
        setEditingCard(null);
        setShowModal(false);
      } else {
        ...
        showSuccess(
          "Pricing Card Added",
          "Your new pricing card has been added successfully.",
        );
        setShowModal(false);
      }
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      showSuccess(
        "Pricing Card Deleted",
        "The pricing card has been deleted successfully.",
      );
    } catch (err) {
  ...
  const handleToggleEnabled = async (card: PricingCard) => {
    try {
      ...
      showError(
        "Error",
        "Failed to update pricing card status. Please try again.",
      );
    }
  };
                ...
                <div className="mb-4">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                      {card.table_rows.length} row
                      {card.table_rows.length !== 1 ? "s" : ""}
                    </div>
                  </div>
                </div>
              )}
      ...
      {pricingCards.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6">
            Create your first pricing card to start displaying rates to
            customers.
          </p>
          <button
            className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium"

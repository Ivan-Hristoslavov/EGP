...
export function EditInvoiceModal({
  ...
  useEffect(() => {
    if (invoice) {
      ...
      if (
        ...
        Array.isArray(invoice.image_attachments)
      ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formDataToSend.append(
      "booking_id",
      useManualEntry ? "" : formData.booking_id,
    );
    ...
    formDataToSend.append(
      "vat_rate",
      vatSettings?.is_enabled ? (vatSettings.vat_rate || 0).toString() : "0",
    );
    ...
    formDataToSend.append(
      "company_address",
      dbProfile?.company_address || "London, UK",
    );
    formDataToSend.append(
      "company_phone",
      dbProfile?.phone || siteConfig.contact.phone,
    );
    formDataToSend.append(
      "company_email",
      dbProfile?.business_email || dbProfile?.email || "",
    );
    formDataToSend.append(
      "company_vat_number",
      vatSettings?.is_enabled ? vatSettings.vat_number || "" : "",
    );
        ...
        <form className="p-6 space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                >
                  ...
                  {filteredBookings.map((booking) => (
                    <option key={booking.id} value={booking.id}>
                      ...
                      {booking.amount.toFixed(2)} ({booking.status})
                    </option>
                  ))}
                </select>
                {errors.booking_id && (
                ...
                {formData.customer_id && filteredBookings.length === 0 && (
                  <p className="text-yellow-600 text-xs mt-1">
                    No available bookings found for this customer. Use manual
                    entry instead.
                  </p>
                )}
              </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
              Add new images or remove existing ones. Images will be
              automatically compressed for faster loading. These images will be
              sent with the invoice email (Max 5 images, 10MB each)
            </p>
                      ...
                      <div key={index} className="relative group">
                        <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-3 border border-gray-200 dark:border-gray-600">
                          ...
                          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            {(file.size / 1024 / 1024).toFixed(1)} MB (will be
                            compressed)
                          </div>
                        </div>
                      </div>
                    ...
                    {existingImages.map((image, index) => (
                      <div key={index} className="relative group">
                        <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-3 border border-gray-200 dark:border-gray-600">
                          ...
                          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            {image.originalSize
                              ? `${(image.originalSize / 1024 / 1024).toFixed(1)} MB`
                              : "Size unknown"}
                            {image.compressedSize && image.compressionRatio && (
                              <span
                                className={`ml-1 ${image.compressionRatio < 0.8 ? "text-green-600" : "text-yellow-600"}`}
                              >
                                (compressed:{" "}
                                {(image.compressedSize / 1024 / 1024).toFixed(
                                  1,
                                )}{" "}
                                MB
                                {image.compressionRatio < 1 &&
                                  `, ${((1 - image.compressionRatio) * 100).toFixed(0)}% smaller`}
                                )
                              </span>
                            )}
                          </div>

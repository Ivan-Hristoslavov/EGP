    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="border border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 sm:p-4">
        ...
        <p className="text-xs sm:text-sm text-emerald-800 dark:text-emerald-200">
          This consultation is free. Confirm your booking below and you&apos;ll
      ...
      {errorMessage && (
        <div className="flex items-start gap-2 sm:gap-3 p-3 sm:p-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
          ...
          <p className="text-red-700 dark:text-red-400 text-sm sm:text-base break-words">
            {errorMessage}
          </p>
        </div>
      )}
    ...
    return (
      <div className="space-y-4 sm:space-y-6">
        <div className="border border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 sm:p-4">
          ...
          <p className="text-xs sm:text-sm text-emerald-800 dark:text-emerald-200">
            This consultation is free. Confirm your booking below and
      ...
      <form className="space-y-4 sm:space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="flex justify-center">
          ...
          >
            {isLoading
              ? "Processing Payment..."
              : `Pay £${chargeAmount.toFixed(2)} Now`}
          </ButtonPrimary>
        </div>
      </form>
      {/* Security Notice */}
...
export default function StripePaymentForm(props: StripePaymentFormProps) {
  ...
  const handleTestBookingMain = async () => {
    ...
    if (
      !props.customerData?.firstName?.trim() ||
      !props.customerData?.email?.trim()
    ) {
      setInitError(
        "Please fill in your details (at least first name and email) before creating a test booking.",
      );
  ...
  const handleInitializePayment = async () => {
    ...
    if (!props.customerData) {
      ...
      setInitError(
        'Please fill in your details in the "Your Details" step before proceeding to payment.',
      );
    ...
    if (!firstName || !email) {
      ...
      setInitError(
        'Please fill in your details (at least first name and email) in the "Your Details" step before proceeding to payment.',
      );
    ...
    console.log("Initializing payment with customer data:", {
      customerName,
      customerEmail,
      customerPhone,
    });
    ...
    try {
      ...
      if (props.depositMetadata?.isDeposit) {
        ...
        metadata.remainingAmount = String(
          props.depositMetadata.remainingAmount,
        );
      }
  ...
  // Calculate total duration
  const totalDuration = props.services.reduce(
    (sum, service) => sum + service.duration * service.quantity,
    0,
  );

  // Format date
  const formattedDate = new Date(props.selectedDate).toLocaleDateString(
    "en-GB",
    {
      weekday: "short",
      day: "numeric",
        ...
        <div className="flex justify-center">
          ...
          >
            {isInitializing
              ...
              : `Pay £${(props.amountToCharge ?? props.amount).toFixed(2)} Now`}
          </ButtonPrimary>
        </div>
        {/* Security Notice */}
        <div className="text-center px-4">

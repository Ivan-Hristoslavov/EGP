...
export function AdminGalleryManager({
  ...
  const filteredServices = useMemo(() => {
    ...
    return services.filter(
      (service) => service.category.id === formData.category_id,
    );
              ...
              <div className="flex items-start justify-between p-6 border-b border-rose-100/60 dark:border-gray-800 sticky top-0 bg-gradient-to-r from-rose-50/90 via-white/90 to-purple-50/90 dark:from-gray-900/95 dark:via-gray-900/95 dark:to-gray-950/95 z-10">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (
                      beforeImagePreview &&
                      beforeImagePreview.startsWith("blob:")
                    ) {
                      URL.revokeObjectURL(beforeImagePreview);
                    }
                    if (
                      afterImagePreview &&
                      afterImagePreview.startsWith("blob:")
                    ) {
                      URL.revokeObjectURL(afterImagePreview);
                    }
                    setBeforeImagePreview("");
                        ...
                        >
                          ...
                          {mainTabs.map((mainTab) => (
                            <optgroup key={mainTab.slug} label={mainTab.name}>
                              {categoriesByMainTab[mainTab.slug]?.map(
                                (category) => (
                  ...
                  <div className="lg:col-span-1 flex flex-col space-y-4 rounded-2xl bg-white/90 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-4 md:p-5 shadow-sm h-full min-h-0">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div
                        className={
                          fieldErrors.before
                            ? "rounded-lg ring-2 ring-red-500 p-1"
                            : ""
                        }
                      >
                      ...
                      (afterImagePreview || formData.after_image_url) && (
                        <div className="mt-4 space-y-3">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setPreviewSliderPosition(Number(e.target.value))
                            }
                          />
                        </div>
                      )}
